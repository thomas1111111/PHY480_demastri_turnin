...
main ()
{
  ...
  ho_parameters.potential_index = 1; //Coulomb potential

  ofstream myout ("eigen_basis.dat");
  ...
  double b_ho = pow(10,-0.8);

  for (int dimension = 1; dimension<26; dimension++)

    {

     for (double r=0; r<10.01; r += 0.05)
     {
           
     ofstream myout ("eigen_basis"+to_string(dimension)+".dat");
      ...
      for (int i = 0; i < dimension; i++)
	{
      ...
      myout << setw (10) << dimension << setw (10) << log10 (dimension) << setw (10) <<
	log10 (fabs ((eigenvalue - (-45.93)) / (-45.93))) << endl;
