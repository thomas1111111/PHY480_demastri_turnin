...
main (void)
{
  ...
  // square two numbers by multiplying them together with an added function
  gettimeofday (&start, NULL);     // start the clock to time the next section
  for (int i = 0; i < repeat; i++)
  {
    x = double(i);
  ...
  cout << "final y = " << y << endl;  // prevent optimizing away loop
  gettimeofday (&stop, NULL);      // stop the clock and print elapsed time
  cout << " Evaluating " << repeat << " squareit took "
    << elapsed (start, stop) << " seconds" << endl << endl;

  // square two numbers by use of a macro
  gettimeofday (&start, NULL);     // start the clock to time the next section
  for (int i = 0; i < repeat; i++)
  {
    x = double(i);
  ...
  cout << "final y = " << y << endl;  // prevent optimizing away loop
  gettimeofday (&stop, NULL);      // stop the clock and print elapsed time
  cout << " Evaluating " << repeat << " the macro took "
    << elapsed (start, stop) << " seconds" << endl << endl;

  // square two numbers by use of an inline function
  gettimeofday (&start, NULL);     // start the clock to time the next section
  for (int i = 0; i < repeat; i++)
  {
    x = double(i);
  ...
  cout << "final y = " << y << endl;  // prevent optimizing away loop
  gettimeofday (&stop, NULL);      // stop the clock and print elapsed time
  cout << " Evaluating " << repeat << " with the inline function took "
    << elapsed (start, stop) << " seconds" << endl << endl;
